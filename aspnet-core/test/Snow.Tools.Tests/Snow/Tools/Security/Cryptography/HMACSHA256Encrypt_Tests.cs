using Shouldly;
using Snow.Tools.Security.Cryptography.Hmacs;
using Xunit;
...
public class HMACSHA256Encrypt_Tests
{
    ...
    {
        using var md5HMAC = new HMACEncrypt(HMACMode.MD5);
        md5HMAC.EncryptToBase64("1234", "1111")
            .ShouldBe("qWtbpRZpotbzKm5Nc51RCA==");

        using var sHA1HMAC = new HMACEncrypt(HMACMode.SHA1); ;
        sHA1HMAC.EncryptToBase64("1234", "1111")
            .ShouldBe("vUFQtOGGgrEai05pTN86TDbQqtk=");

        using var sHA256HMAC = new HMACEncrypt(HMACMode.SHA256);
        sHA256HMAC.EncryptToBase64("1234", "1111")
            .ShouldBe("OIRyAA0CHB99CJxkcnUM34C5F3Xdf/aaWdMkIWzf8V4=");

        using var sHA384HMAC = new HMACEncrypt(HMACMode.SHA384);
        sHA384HMAC.EncryptToBase64("1234", "1111")
            .ShouldBe("Bemy8D5Yqh/p7kDreTkBQ0HKl5ajCepMoz3gblxj02R1KoPOTIb1JDqlVE5/IvcY");

        using var sHA512HMAC = new HMACEncrypt(HMACMode.SHA512);
        sHA512HMAC.EncryptToBase64("1234", "1111")
